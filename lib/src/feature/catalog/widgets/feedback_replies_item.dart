                        ...
                        itemBuilder: (context, subindex) {
                          return ReplyFeedbackItem(
                            ...
                            selectedFeedback: (name, parentId, isAnswerBottomSheet) {
                              widget.selectedFeedback.call(name, parentId, isAnswerBottomSheet);
                            },
                            onAnswerTapped: () {
    ...
    return Column(
      children: [
        Row(
          ...
          children: [
            ReviewAvatar(
              imageAva: reply?.user?.avatar ?? replyTwo?.user?.avatar ?? NOT_FOUND_IMAGE,
                    ...
                    children: [
                      Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Text(reply?.user?.name ?? replyTwo?.user?.name ?? '',
                                  style: AppTextStyles.fs14w600.copyWith(color: const Color(0xff605b5b), height: 1.3)),
                              const Gap(4),
                              if (replyTwo != null) SvgPicture.asset(AssetsConstants.next),
                              const Gap(4),
                              if (replyTwo != null)
                                Text(parentName,
                  ...
                  return ReplyFeedbackItem(
                    ...
                    onAnswerTapped: () {
                      selectedFeedback.call(
                          reply?.reply?[subindex].user?.name ?? '', reply?.reply?[subindex].id ?? 0, true);
                    },
                    selectedFeedback: (name, parentId, isAnswerBottomSheet) {
            ...
            ListView.separated(
                ...
                itemBuilder: (context, subindex) {
                  return ReplyFeedbackItem(
                    ...
                    onAnswerTapped: () {
                      selectedFeedback.call(
                          replyTwo?.reply?[subindex].user?.name ?? '', replyTwo?.reply?[subindex].parentId ?? 0, true);
                    },
                    selectedFeedback: (name, parentId, isAnswerBottomSheet) {
                      selectedFeedback.call(name, parentId, isAnswerBottomSheet);
                    },
